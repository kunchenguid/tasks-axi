...
Archive lookup is strictly read-only: it never restores, rewrites, or otherwise mutates cold history, and it recognizes only canonical `## Archived YYYY-MM-DD` blocks written by pruning. A malformed task-shaped record for the requested id fails with `VALIDATION_ERROR` instead of being skipped or hidden by fallback, while multiple valid archive records for that id fail with `CONFLICT`. If neither surface contains the id, `show` keeps the existing `NOT_FOUND` response and exit code.
Archived Done records retain their stored `deps`, but report `blocked: no` and `blocked_by: none` because blocker state applies only to active work.
