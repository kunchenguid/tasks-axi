...
Archive lookup is strictly read-only: it never restores, rewrites, or otherwise mutates cold history. An active identity always wins, a structurally invalid active record is never masked by an archived copy, and duplicate matching identities in the archive fail as a conflict instead of selecting one silently. If neither surface contains the id, `show` keeps the existing `NOT_FOUND` response and exit code.
