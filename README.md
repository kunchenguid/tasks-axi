...
`mv <id> [<id>...] --to <path-or-dir>` validates the complete set under both backlog locks before writing. Validation failures leave both files unchanged. It writes the destination first, then the source. If source persistence fails, destination rollback removes tasks one at a time. If rollback fails, the command reports PARTIAL_MOVE (error code `CONFLICT`, message `Move of "<id>" partially completed; task now exists in both backlogs`). For multi-ID moves, that generic message does not mean every ID is duplicated: rollback can leave some or all requested tasks at the destination while their source copies remain.

The move is not crash-atomic. Process interruption between writes can leave copies in both backlogs; retrying `mv` then refuses with `CONFLICT`. Inspect both files and reconcile the entire dependency-connected set before deleting any source copies. To keep the source set, remove all remaining moved copies from the destination. To keep the destination, first ensure every moved task and linked dependency endpoint is present there; only then remove its source copies. Remove active dependents before blockers in whichever backlog you clean, using `tasks-axi rm <id>` against that backlog. Power loss and storage failure have no durability guarantee from this write protocol: the atomic write is a temp file plus `rename`, with no `fsync`.
