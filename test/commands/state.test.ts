...
import { FakeAdoClient } from "../backends/fake-ado.js";
...
describe("state commands", () => {
	...
	describe("mv", () => {
		it("rejects unsupported file moves before touching the destination", async () => {
			...
			await store.create({ id: "remote-mv-a1", title: "remote" });
			const target = makeBacklog("# Backlog\n\n## Queued\n\n## Done\n");
			const before = target.read();
			const ctx: TasksContext = {
				store,
				config: { backend: "ado", path: "ado", doneKeep: 10 },
			};
			try {
				await expect(
					mvCommand(["remote-mv-a1", "--to", target.path], ctx),
				).rejects.toMatchObject({
					code: "UNSUPPORTED",
					suggestions: expect.arrayContaining([
						expect.stringContaining("area-path moves"),
					]),
				});
				expect(target.read()).toBe(before);
				expect(await store.get("remote-mv-a1")).not.toBeNull();
				expect(
					client.calls.filter((call) => call.kind === "remove"),
				).toHaveLength(0);
			} finally {
				target.cleanup();
			}
