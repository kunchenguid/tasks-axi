...
describe("resolveConfig", () => {
	...
	it("rejects unknown ADO keys only when ADO is selected", () => {
		...
		).toThrow(/Unknown ado config key: aera/);
	});
	...
	describe("[ado] table", () => {
		...
		// A doubled separator would quietly point every WIQL read at an area path
		// ADO does not have, regardless of where the resolved value came from.
		it.each([
			["basic TOML", 'area = "Internal\\\\Firstmate"', {}],
			["literal TOML", "area = 'Internal\\\\\\\\Firstmate'", {}],
			[
				"environment",
				"area = 'Internal\\Firstmate'",
				{ TASKS_AXI_ADO_AREA: "Internal\\\\Firstmate" },
			],
		])("refuses doubled separators from %s", (_source, areaLine, env) => {
			writeFileSync(
				join(dir, ".tasks.toml"),
				[
					'backend = "ado"',
			...
			expect(() => resolveConfig({ cwd: dir, home, env })).toThrow(
				/must use single backslash separators/,
			);
		});
