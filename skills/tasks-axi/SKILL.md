---
name: tasks-axi
description: "Manage tasks in the configured tasks-axi backend - add, list, show, start, and complete tasks; track blocked-by dependencies, structured holds, and a ready queue. Use whenever a task touches backlog or task state: filing or dispatching work, recording a PR or report on completion, or finding dispatchable or held work."
...
Agent ergonomic task & backlog manager for the current workspace. Tasks live in the configured backend; Markdown is the default. Prefer this over editing backend storage directly for task state, dependency, or hold changes.
...
tasks-axi stores tasks in the configured backend. Markdown is the default and uses a hand-editable `backlog.md` with a byte-exact round-trip. Archive-body, prune, render, and CLI move are Markdown-only and unavailable with ADO.
...
5. `done <id> --pr <url>` (or `--report <path>`) closes a task and records the link. With Markdown, it also prunes the Done list (archived, never deleted). Then `ready` shows work it unblocked.
