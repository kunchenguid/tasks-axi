...
- **`mv` is a multi-id cross-file move, not a crash-atomic transaction.** `moveManyTo` validates the set under both backlog locks, then persists destination and source in that order. Source-persistence errors trigger a rollback attempt; rollback failure reports PARTIAL_MOVE (error code `CONFLICT`, "task now exists in both backlogs"). Process interruption between writes can leave duplicates, and re-running `mv` then refuses with `CONFLICT` until the source copy is removed by hand. Storage durability is not guaranteed: `atomicWrite` is a temp file plus `rename`, with no `fsync`. Closing the interruption window needs a write-ahead intent plus a resume step. Intra-set `blocked-by` edges (reason strings included) survive because both endpoints travel together; `requireNoSplitDeps` refuses and names any edge whose blocker/dependent would be stranded across the two files. Single-id `mv` is just N=1 (`moveTo` delegates to `moveManyTo`), so its byte output is unchanged. Moved items are re-rendered canonically, so trailing blank separators before the next item/section are dropped (a move-then-move-back is byte-exact only when the source had no such trailing blank).
