...
- **`mv` safety and recovery.** See README.md's “The markdown backend” section for the persistence, rollback, duplicate-reconciliation, and durability contract. Intra-set `blocked-by` edges (reason strings included) survive because both endpoints travel together; `requireNoSplitDeps` refuses and names any edge whose blocker/dependent would be stranded across the two files. Single-id `mv` delegates to `moveManyTo`; moved items are re-rendered canonically, so trailing blank separators before the next item/section are dropped.
