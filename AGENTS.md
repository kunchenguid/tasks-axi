...
- **`list` is the dispatch view by default.** A bare `tasks-axi list` shows only ready work (unblocked, unheld queued tasks), priority-ranked, one short `{id,state,priority}` line per task, so queue re-evaluations stay cheap in supervisor context. `--compact` forces that one-line schema under any flags, and `--all` restores the full backlog listing (every state, full schema). Every other existing flag (`--state`/`--repo`/`--kind`/`--blocked`/`--limit`/`--fields`) keeps the full-schema output exactly as before, so fm-side digest and desk scripts that always pass flags are unaffected.
