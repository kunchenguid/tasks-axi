...
- **`moveTo(id, areaPath)`** is the cross-queue move (Area Path change). Cross-scope moves of connected tasks are refused; the CLI `mv` verb is still markdown/path-shaped, and wiring safe area-path moves is a follow-up.
