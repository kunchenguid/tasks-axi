...
- **The optional start fence is enforced only at `MarkdownStore.transition` for queued → in-flight.** `start_fence` / `TASKS_AXI_START_FENCE` names an opaque marker; relative paths resolve beside the active backlog. The check uses `lstat`-style presence (including directories and broken symlinks), never reads or changes the marker, fails closed on inspection errors, and does no marker I/O when unconfigured.
