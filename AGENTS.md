...
- **ADO owns what ADO models**, tasks-axi owns the rest: title/state/area/description/tags/links are native fields, with the body HTML-encoded in `System.Description` and decoded on read so ADO renders it as plain text; the residue (priority, hold, typed links, created/closed) rides in one JSON `meta_field`. The public-followup obligation gets its own field carrying the same canonical base64url payload the markdown backend writes, so a Relay obligation reads identically on either backend.
