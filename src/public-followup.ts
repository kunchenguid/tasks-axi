...
export function assertPublicFollowupTaskContent(
  task: { title: string; body?: string; links: readonly unknown[] },
  value: PublicFollowup,
): void {
  if (
    task.title !== value.request.public_safe_summary ||
    task.body !== undefined ||
    task.links.length > 0
  ) {
    validation(
      "public-followup task content must match its public-safe promise",
    );
  }
}
